...
public sealed record StoreEvent()
{
    ...
    private StoreEvent(Guid id, Guid streamId, string eventType, IDomainEvent data, DateTimeOffset timestamp,
        long version) : this()
    {
        Id = id;
        StreamId = streamId;
    ...
    {
        return new StoreEvent(@event.MessageId, aggregate.Id, @event.GetType().FullName!, @event, @event.Timestamp,
            @event.Version);
    }
}