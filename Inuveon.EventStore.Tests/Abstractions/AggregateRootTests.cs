...
public class AggregateRootTests
{
    ...
    public void LoadFromHistory_AppliesEventsCorrectly()
    {
        ...
        {
            new UserRegistered(Guid.NewGuid(), aggregateId, TestDataGenerator.RandomFirstName(), emailAddress,
                DateTimeOffset.Now, 1, Guid.NewGuid(), Guid.NewGuid()),
            new UserNameChanged(Guid.NewGuid(), aggregateId, updatedFirstName, DateTimeOffset.Now, 2, Guid.NewGuid(),
                Guid.NewGuid())
        };
    ...
    {
        var aggregate = new UserAggregate();
        var command = new RegisterUserCommand(Guid.NewGuid(), TestDataGenerator.RandomFirstName(),
            TestDataGenerator.RandomEmail(), DateTimeOffset.Now, 0, Guid.NewGuid());
    ...
    {
        var aggregate = new UserAggregate();
        var command = new RegisterUserCommand(Guid.NewGuid(), "", TestDataGenerator.RandomEmail(), DateTimeOffset.Now,
            0, Guid.NewGuid());
    ...
    public void ShouldThrowValidationException_WhenEmailIsMissing()
    {
        var aggregate = new UserAggregate();
        var command = new RegisterUserCommand(Guid.NewGuid(), TestDataGenerator.RandomFirstName(), "",
            DateTimeOffset.Now, 0, Guid.NewGuid());
    ...
    {
        var aggregate = new UserAggregate();
        var command = new RegisterUserCommand(Guid.NewGuid(), TestDataGenerator.RandomFirstName(), "wrong",
            DateTimeOffset.Now, 0, Guid.NewGuid());
